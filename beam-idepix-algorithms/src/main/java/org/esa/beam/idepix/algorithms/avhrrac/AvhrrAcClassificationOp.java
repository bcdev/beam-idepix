...
public abstract class AvhrrAcClassificationOp extends Operator {
    ...
    void setPixelSamples(Band band, Tile targetTile, int y, int x, AvhrrAcAlgorithm globAlbedoAlgorithm) {
    ...
    void setCloudFlag(Tile targetTile, int y, int x, AvhrrAcAlgorithm globAlbedoAlgorithm) {
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    if (targetTile.getSampleBit(x, y, IdepixConstants.F_CLOUD)) {
                        int LEFT_BORDER = Math.max(x - aacCloudBufferWidth, rectangle.x);
                        int RIGHT_BORDER = Math.min(x + aacCloudBufferWidth, rectangle.x + rectangle.width - 1);
                        int TOP_BORDER = Math.max(y - aacCloudBufferWidth, rectangle.y);
                        int BOTTOM_BORDER = Math.min(y + aacCloudBufferWidth, rectangle.y + rectangle.height - 1);
